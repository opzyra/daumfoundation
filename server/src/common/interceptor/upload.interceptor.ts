...
import { uploadStorage } from 'src/providers/multer/multer.storage';
...
export const UploadInterceptor = (context?: string) =>
  UseInterceptors(
    FileInterceptor('file', {
      storage: uploadStorage(context),
      limits: {
        files: 1,
        fileSize: convertFileSize(process.env.UPLOAD_MAX_SIZE),
      },
      fileFilter: (req, file: any, callback) => {
        ...
        const extension = fileName
          ...
          .toLowerCase();
        const whitelist = process.env.UPLOAD_EXTENSION.split(',');
