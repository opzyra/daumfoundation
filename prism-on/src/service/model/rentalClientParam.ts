...
import type { RentalClientParamStatus } from './rentalClientParamStatus';
import type { UserDto } from './userDto';

export interface RentalClientParam {
  /** 대여 번호 8자리 */
  rentalNo?: number;
  /** 상태 */
  status?: RentalClientParamStatus;
  /** 대여 희망 기간 */
  period?: number;
  /** 승인 대여 기간 */
  ...
  createdAt?: string;
  /** 복지카드 PK */
  welfareCardId: number;
  /** 신분증 PK */
  residentCardId: number;
  /** 청각검사 결과지 PK */
  hearingTestId: number;
  /** 가족관계 증명서 PK */
  familyCertificateId: number;
  /** 취약계층 증빙서류 PK */
  vulnerableProofId: number;
  /** 수리불가 판정서 PK */
  unrepairableCertificateId: number;
  /** 분실신고 증빙서류 PK */
  lossReportId: number;
}
